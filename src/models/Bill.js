...
const billSchema = new mongoose.Schema(
  ...
  },
  {
    ...
    collection: 'bills',                               // explicit MongoDB collection name
  }
);

// Index on userId to quickly list all bills for a user
billSchema.index({ userId: 1 });
// Compound index: fetch only pending or overdue bills for a user
billSchema.index({ userId: 1, status: 1 });
// Index on dueDate: useful for finding bills coming due soon
